...
const TELEGRAM_BOT_TOKEN = null; // Get from @BotFather
const TELEGRAM_CHAT_ID = null;    // Get from @userinfobot
...
function log(msg, type = 'INFO') {
  const colors = { INFO: '\x1b[36m', BUY: '\x1b[32m', SELL: '\x1b[33m', ERROR: '\x1b[31m', SUCCESS: '\x1b[32m', LEARN: '\x1b[35m', TG: '\x1b[34m' };
...
async function sendTelegram(message) {
  ...
  try {
    ...
    log('📱 Telegram sent', 'TG');
  } catch (e) {
    log('Telegram error: ' + e.message, 'ERROR');
  }
}

async function notifyBuy(coin, amount) {
  const msg = `🟢 <b>BUY</b>\nCoin: ${coin.symbol}\nAmount: $${amount.toFixed(2)}\nCapital: $${state.capital.toFixed(2)}`;
  await sendTelegram(msg);
}

async function notifySell(coin, amount, pnl, pnlPct, won) {
  const emoji = won ? '✅' : '❌';
  const label = won ? 'WIN' : 'LOSS';
  const msg = `${emoji} <b>SELL - ${label}</b>\nCoin: ${coin.symbol}\nPnl: $${pnl.toFixed(2)} (${pnlPct.toFixed(2)}%)\nTotal Capital: $${state.capital.toFixed(2)}`;
  await sendTelegram(msg);
}
...
async function executeTrade() {
  ...
  const bias = (historicalWinRate - 0.5) * 0.05;
  ...
  await notifySell(coin, tradeUsd, result.pnl, result.priceChange, result.won);
...
async function main() {
  ...
  if (TELEGRAM_BOT_TOKEN && TELEGRAM_CHAT_ID) {
    log('📱 Telegram notifications ENABLED', 'TG');
    await sendTelegram('🤖 Bot started!\nMode: ' + (CONFIG.PAPER_MODE ? 'PAPER' : 'LIVE') + '\nCapital: $' + state.capital);
  } else {
    log('⚠️ Telegram not configured - set TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID', 'INFO');
  }
